...
민감정보를 이 리포지토리의 `helm/cmdb-core/values.yaml`에 직접 쓰지 않는다. 레이아웃은
[values-prod.yaml.example](values-prod.yaml.example)에 있으니, 이 파일을 복사해서 폐쇄망
안에서 직접 실제 값을 채운다 (비밀번호 등은 외부망에서 들고 들어가지 않는다):

```powershell
cp values-prod.yaml.example values-prod.yaml
# values-prod.yaml 값 채우기
```

`values-prod.yaml`은 `.gitignore`에 등록돼 있어 git에 커밋되지 않는다.
