...
- **DB(Oracle) 정보 수집 신규 기능(1.0.41)**: "WEB/WAS/OS/시스템처럼 DB(오라클 12c/19c)도 자동 수집하고 싶다, AWX든 CMDB 직접 수집이든 상관없다"는 요청 — 바로 코딩하지 않고 먼저 CMDB가 외부 대상에 직접 접속하지 않는 기존 원칙(vCenter/Nutanix도 AWX가 대신 조회)을 근거로 AWX push 방식을 제안하고 이유를 설명, 사용자 승인 후 설계 확정을 위해 `AskUserQuestion`으로 3가지 확인(RAC 지원 여부 → "처음부터 지원", 수집 범위 → "인스턴스 메타정보만", 인증 방식 → "OS 인증(로컬 sqlplus)") 후 진행. 이어서 "OS 수집처럼 소스 변경 없이 필드 추가 가능하게 해달라(field definitions 지정)"는 추가 요청까지 받아 설계에 반영
  - `was`/`systems` 앱 코드를 먼저 재확인해 두 패턴을 조합 — WAS 패턴(소스=도메인 하나, 컨테이너=인스턴스, admin 호스트 자기 식별)과 systems 패턴(SystemHost의 `extra` JSON + `SystemHostFieldDefinition` 동적 필드)을 합쳐 신규 `database` 앱 설계: `DbConfigSource`(DB 하나, RAC 포함 - **매칭 키가 asset이 아니라 Oracle이 보장하는 전역 유일값 `db_unique_name`**, RAC는 대표 노드가 push마다 바뀔 수 있어 asset을 키로 못 씀), `DbInstance`(인스턴스=SID 하나, RAC는 GV$INSTANCE 조회 결과만큼 여러 행, push마다 통짜 교체), `DbConfigSourceFieldDefinition`/`Value`/`Choice`(SystemHostFieldDefinition과 완전히 동일한 AUTO/MANUAL+CHOICE 구조, `DbConfigSource.extra`에서 dot-path 추출)
  - AWX 쪽은 Oracle 12c/19c 둘 다 지원하는 `JSON_OBJECT`/`JSON_ARRAYAGG` SQL 함수로 sqlplus가 직접 CMDB가 기대하는 snake_case JSON을 출력하도록 설계(WebToB/Apache처럼 텍스트 정규식 파싱이 필요 없어짐) - `RETURNING CLOB`으로 4000바이트 제한 회피, `SYS_CONTEXT('USERENV','INSTANCE_NAME')`을 `collected_from_instance`로 실어보내 WAS의 admin-server-name과 같은 방식으로 "이번 push를 실행한 노드"를 결정적으로 식별. `push_oracle_config_to_cmdb.yml` 작성, `awx/README.md`에 6번 섹션(Job Template) 신설하며 이후 번호 전부 재정렬
  - 실제 API push로 검증하다가 **버그 발견**: `database/parsers.py`가 `collected_from_instance`를 최상위로 승격하지 않고 `extra` 안에만 남겨둬서 `DbConfigSource.asset` 매칭이 계속 null로 나옴 — 원인 확인 후 즉시 수정, 재검증으로 Standalone(`drnrap01`)/RAC 2노드(`ddorap01`/`iawxap01`) 둘 다 asset 정상 매칭 확인
  - 동적 필드 확장성을 실제로 증명: 임의로 `force_logging` AUTO 필드를 admin(shell)에서 등록 → 코드 변경/재push 없이 "선택한 필드 소급 백필"만으로 이미 push된 DB 2건에 즉시 값이 채워지는 것 확인, MANUAL 필드(`담당자`)도 대시보드 인라인 편집으로 저장 후 엑셀 다운로드에 같이 나오는 것까지 확인
  - `dashboard` 통합(queries.py 쿼리 4종, views.py 뷰 9개, list_export.py 2종 + `database/excel_import.py` 신규, 템플릿 5개, urls.py, base.html DB 드롭다운)까지 전부 Django test client로 실제 렌더링(200 OK) 검증, 기존 화면(자산/웹설정/WAS/시스템/서비스 목록) 회귀 없음도 재확인
  - `samples/oracle/`(Standalone 19c + RAC 12c 샘플, `README.md`)와 CLAUDE.md에 "데이터베이스" 섹션 신설 + "대시보드"(IP 컬럼 8개 화면으로 확장)/"테스트·검증"(샘플 안내) 섹션 갱신 - **이 저장소 개발 환경에 실제 Oracle 인스턴스가 없어 AWX SQL(특히 listener_port 정규식)은 미검증 상태임을 CLAUDE.md/README 양쪽에 명시**(vCenter 하드웨어 스펙 케이스와 같은 정직한 기록 원칙)
  - 검증 스크립트가 `docker compose cp`로 컨테이너에 들어가며 호스트 루트에도 새어나온 걸 매번 확인 후 정리(이번 세션 초반에 확립한 습관 재사용)
  - 커밋 전 확인 단계에서 사용자 질문 두 건 처리(코드 변경 없이 설명만): "RAC면 DB1/DB2 각각 push해야 하나?" → `gv$instance`가 어느 노드에서 조회하든 클러스터 전체를 돌려주는 특성상 대표 노드 1대 push로 충분하다고 답변(중복 push해도 upsert라 안 깨짐도 같이 설명). "인스턴스=SID였구나" → 맞다고 확인, 개념 정리
...
- **DB(Oracle)/WAS↔DB 교차 연결 내부망 실측 검증 성공**: 1.0.42 반입 후 사용자가 "DB push는 잘 되는데 WAS↔DB 매핑이 안 된다"고 보고 — 코드 버그가 아니라 **push 순서 문제**로 판단해 설명(매칭은 WAS push 시점에 그때 존재하는 DB 목록과 대조하는 구조라, DB push가 실패했던 1.0.41 시점에 이미 했던 WAS push는 매칭 대상 자체가 없었을 것) — WAS Job Template을 DB push 성공 이후 재실행해보라고 안내
  - 재push 후 사용자가 스크린샷 2장 공유: WAS 상세의 Data Source 표에 "연결된 DB: DRNRODBS / DRNRODBS" 정상 표시, DB 인스턴스 목록에도 `DRNRAP01`/`DRNRODBS`/OPEN/19.0.0.0.0/**리스너 포트 1523**까지 전부 정상 수집 — 안내대로 재push하니 매칭 성공, 가설 확인
  - 그동안 "미검증"으로 문서에 남겨뒀던 `listener_port` 정규식 추출이 실제로 정확히 동작하는 것까지 이번에 실측 확인 — CLAUDE.md의 해당 캐비어트를 "실측 검증 완료"로 갱신(코드 변경 없는 문서 전용 커밋으로 준비했다가 아래 구성도 작업과 합쳐서 같이 커밋)
- **서비스 구성도에 DB 노드 추가(1.0.43)**: WAS↔DB 매칭 확인 스크린샷을 보다가 사용자가 "구성도에는 그림으로 표기가 안 되는데 별도 작업이 필요하냐"고 질문 — `dashboard/topology.py`를 확인해 실제로 DB를 전혀 모른다는 걸 확인하고 바로 구현
  - DB는 서비스에 직접 배정되는 대상이 아니라서(`DbConfigSource`/`DbInstance`엔 `service` 필드 자체가 없음) 이미 그래프에 있는 WAS 컨테이너가 참조하는 `JeusDataSource.db_instance`(push 시점에 이미 해석된 값)를 따라가는 방식으로 설계 — WebToB<->JEUS 엣지(`JeusWebtobConnector`)와 완전히 같은 원칙(서비스 배정이 아니라 실제 연결을 따라감). 같은 DB 인스턴스를 여러 컨테이너가 공유하면 노드는 하나만, 엣지만 컨테이너 수만큼
  - 로컬 데이터로 실제 서비스(`CMP Web Service`, DRNRAP01_CMP1 컨테이너가 이미 이 서비스에 배정돼 있었음)를 대상으로 그래프 생성 함수를 직접 호출해 노드 3개(WebToB v_cmp → JEUS DRNRAP01_CMP1 → Oracle DRNRODBS)와 엣지 2개가 정확히 나오는 것, SVG에 DB 라벨이 포함되는 것, 대시보드 뷰(`/dashboard/services/topology/?name=...`)로도 200 + DB 정보 포함 확인까지 Django test client로 검증
  - CLAUDE.md "구성도" 섹션에 DB 노드 편입 방식 문서화
  - 1.0.43 커밋 확인을 요청했으나, 사용자가 곧바로 WAS 상세 화면 스크린샷과 함께 후속 요청 4건을 먼저 이어가 커밋은 보류된 채로 다음 작업으로 넘어감(아래)
- **상세 모달 간 드릴다운 추가(1.0.43에 이어서 포함)**: WAS 상세 스크린샷을 보다가 사용자가 4가지 요청 — (1) "연결된 DB"에 SID가 두 번 나오는 거 하나로 (2) 자산 상세→시스템 링크가 페이지 이동인데 모달로 (3) 시스템 상세→OS 링크도 마찬가지로 모달로 (4) WAS 상세의 "연결된 WEB/DB"도 가능하면 모달로 — "코딩 전에 검토부터"라는 명시적 요청이 있어 바로 구현하지 않고 먼저 설계 검토
  - CLAUDE.md에 이미 "중첩 모달 회피"가 여러 군데 명시된 기존 원칙(자산/시스템 상세를 읽기 전용으로 둔 이유)이라는 걸 짚고, 진짜 모달 위에 모달을 쌓는 대신 서비스 조회 화면(`webservice_list.html`)에 이미 있던 "모달 하나, 클릭한 대상에 따라 내용만 교체"(`data-detail-url`/`data-content-id`) 패턴을 재사용하는 방향을 제안 — 남은 결정 2가지(WAS의 "연결된 WEB/DB" 클릭 시 자산 상세를 보여줄지 WEB/DB 설정 상세를 보여줄지, 여러 단계 들어갔을 때 "뒤로가기"가 필요한지)는 `AskUserQuestion`으로 확인: "WEB/DB 설정 상세(추천)", "뒤로가기 없이 닫기만" 선택받고 진행
  - `asset_detail.html`/`system_detail.html`/`was_detail.html`의 대상 링크를 전부 `<a href>`에서 `data-detail-url`/`data-content-id`/`data-detail-title` 속성을 가진 클릭 가능한 span으로 교체, `asset_list.html`/`system_list.html`/`was_list.html`의 모달 JS에 `loadIntoModal()`+`bindDrilldownLinks()`(주입된 내용 안의 `[data-detail-url]`을 다시 델리게이션 바인딩)를 추가 - 뒤로가기 없이 계속 갈아끼워지는 방식이라 상태 관리가 단순함. 같은 김에 "연결된 DB"는 `db_unique_name`만 보여주도록 정리
  - Playwright(로컬 `.venv`에 이미 설치돼 있던 걸 발견)로 실제 브라우저 클릭까지 검증 — 자산 모달 → 시스템 드릴다운 → 다시 자산 드릴다운 왕복 후에도 열린 모달이 항상 1개뿐인 것, WAS 모달 → DB 드릴다운 클릭 시 제목이 "DB 상세"로 바뀌고 인스턴스 정보가 로드되는 것까지 확인. 첫 실행 결과가 한글 깨짐(콘솔 인코딩 문제)으로 나와 `PYTHONIOENCODING=utf-8 python -X utf8`로 재실행해 깨끗한 로그 확보
  - **사용자 피드백으로 설계 정정**: "내가 원한 거랑 다르다, 모달에 모달을 또 띄우는 걸 생각했다 — 메인 대시보드 그래프에서 리스트 누르고 상세 나오는 것처럼"이라는 지적 — 통합대시보드 도넛 드릴다운(1.0.39)의 목록 모달(iframe)을 다시 확인해 "그건 검색/정렬 가능한 전체 목록 화면을 통째로 넣어야 해서 iframe을 쓴 것"임을 파악하고, 여기 대상(자산/시스템/WAS 상세)은 자체 기능 없는 단순 상세라 iframe 없이 별도 `.modal` 엘리먼트 하나(`crossDetailModal`, z-index만 명시)를 추가해 첫 모달 위에 진짜로 쌓는 방식으로 재설계 제안 → 승인받고 재구현. `loadIntoModal()`(내용 갈아끼우기)를 걷어내고 `openCrossDetail()`(두 번째 모달을 새로 열기)로 교체, 세 번째 이상 중첩은 막기 위해 교차 모달 안에서의 추가 클릭은 그 교차 모달 자신의 내용만 다시 갈아끼우도록 함(첫 모달은 항상 고정 복귀 지점)
  - Playwright로 재검증 — 두 모달이 동시에 `is-active`인 것, 아래(첫) 모달의 내용이 위 모달을 열어도 안 바뀌고 그대로인 것, 위 모달을 닫으면 아래 모달이 그대로 남아있는 것(자동 복귀)까지 실제 클릭으로 확인. 스크린샷으로 시각적 레이어링(아래 모달이 어둡게 깔리고 위에 새 모달이 얹히는 모습)도 직접 확인
  - CLAUDE.md "대시보드 디자인 패턴"에 `crossDetailModal` 패턴 신규 문서화(편집 폼이 아니라 읽기 전용 상세 간 이동에만 쓴다는 점 명시)
  - **버전 관리 절차 버그 발견 및 수정**: 위 작업을 마치고 1.0.43 이미지를 다시 빌드했더니 크기가 331MB → 817MB로 뻥튀기됨 — `docker history`로 레이어별 크기를 확인해 `COPY . .` 레이어 하나가 511MB인 것을 발견, 원인은 지난 1.0.43 작업 때 커밋 확인을 못 받고 바로 다음 요청(이 드릴다운 작업)으로 넘어가면서 `cmdb-core-1.0.43.tar`/`.tar.zip`(약 485MB)를 정리하지 못한 채 리포지토리 루트에 남아있었고, `.dockerignore`가 `*.tar`/`*.tar.zip`을 걸러내지 않아 `docker build`가 그대로 이미지 안에 다시 넣어버린 것 — `.dockerignore`에 두 패턴 추가해 근본 수정, 남아있던 모든 버전의 stray tar/zip과 오염된 로컬 이미지(1.0.43/1.0.44 둘 다 잘못 빌드됨)를 지우고 재빌드해 331MB로 정상 확인. 버전 번호도 정리 — 실제 GitHub에 릴리즈된 마지막 버전은 v1.0.42뿐이었는데(1.0.43은 커밋 확인을 못 받아 릴리즈까지 못 감) 무심코 VERSION을 1.0.44까지 두 번 올렸던 걸 1.0.43 하나로 되돌리고, 구성도+드릴다운 두 기능을 한 번에 1.0.43으로 묶어서 릴리즈하기로 정리
  - 1.0.43로 VERSION/CHANGELOG 갱신(구성도+드릴다운+실측검증기록 전부 포함) → `docker build`(정상 크기 331MB 확인)/`save`/zip 압축 → 커밋 확인 → push → GitHub Release 생성까지 통상 절차대로 진행
